...
export class EvenementMachineService {

  private className = '/api/arretMachine/evenement_machine';
  ...
  bulkCreate(entityObj) {
    const path = '/bulkCreate';
    return this.httpMethode.post(this.className + path, entityObj);
  }
  ...
  findAllMachineFabricationSteps() {
    const path = '/findAllMachineFabricationSteps';
    return this.httpMethode.get(this.className + path);
  }
}
